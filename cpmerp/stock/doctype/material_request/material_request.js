...
frappe.ui.form.on("Material Request", {
	...
	update_status: function (frm, stop_status) {
		frappe.call({
			method: "cpmerp.stock.doctype.material_request.material_request.update_status",
		...
		frappe.prompt(
			{
				...
				get_query: () => {
					return {
						query: "cpmerp.stock.doctype.material_request.material_request.get_default_supplier_query",
			...
			(values) => {
				frappe.model.open_mapped_doc({
					method: "cpmerp.stock.doctype.material_request.material_request.make_purchase_order",
					frm: frm,
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_in_transit_stock_entry(frm) {
		frappe.prompt(
			...
			(values) => {
				frappe.call({
					method: "cpmerp.stock.doctype.material_request.material_request.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		frappe.model.open_mapped_doc({
			method: "cpmerp.stock.doctype.material_request.material_request.create_pick_list",
			frm: frm,
		});
	},
	...
	raise_work_orders: function (frm) {
		frappe.call({
			method: "cpmerp.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
	...
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				cpmerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
cpmerp.buying.MaterialRequestController = class MaterialRequestController extends (
	cpmerp.buying.BuyingController
) {
	tc_name() {
		this.get_terms();
	}
...
extend_cscript(cur_frm.cscript, new cpmerp.buying.MaterialRequestController({ frm: cur_frm }));

function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		cpmerp.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
