...
cpmerp.accounts.taxes.setup_tax_validations("Purchase Receipt");
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "cpmerp.stock.doctype.purchase_receipt.purchase_receipt.make_inter_company_delivery_note",
						frm: cur_frm,
					});
				},
				__("Create")
...
cpmerp.stock.PurchaseReceiptController = class PurchaseReceiptController extends (
			...
			frappe.prompt(
				...
				function (values) {
					if (values.return_for_rejected_warehouse) {
						frappe.call({
							method: "cpmerp.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return_against_rejected_warehouse",
							args: {
	...
	apply_putaway_rule() {
		if (this.frm.doc.apply_putaway_rule) cpmerp.apply_putaway_rule(this.frm);
	}
};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new cpmerp.stock.PurchaseReceiptController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (status) {
	frappe.ui.form.is_saving = true;
	frappe.call({
		method: "cpmerp.stock.doctype.purchase_receipt.purchase_receipt.update_purchase_receipt_status",
...
cur_frm.cscript._make_purchase_return = function () {
	frappe.model.open_mapped_doc({
		method: "cpmerp.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return",
		frm: cur_frm,
	});
};
...
cur_frm.cscript["Make Stock Entry"] = function () {
	frappe.model.open_mapped_doc({
		method: "cpmerp.stock.doctype.purchase_receipt.purchase_receipt.make_stock_entry",
		frm: cur_frm,
	});
};
