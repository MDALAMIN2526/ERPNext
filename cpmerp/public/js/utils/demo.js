...
function render_clear_demo_action() {
	let demo_action = $(
		`<a class="dropdown-item" onclick="return cpmerp.demo.clear_demo()">
			${__("Clear Demo Data")}
		</a>`
	);
...
cpmerp.demo.clear_demo = function () {
	frappe.confirm(__("Are you sure you want to clear all demo data?"), () => {
		frappe.call({
			method: "cpmerp.setup.demo.clear_demo_data",
			freeze: true,
