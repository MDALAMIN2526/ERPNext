...
cpmerp.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new cpmerp.accounts.NumberCard(summary);
...
cpmerp.accounts.NumberCard = class NumberCard {
	constructor(options) {
