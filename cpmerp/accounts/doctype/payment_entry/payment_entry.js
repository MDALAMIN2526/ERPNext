...
cpmerp.accounts.taxes.setup_tax_validations("Payment Entry");
cpmerp.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");
...
frappe.ui.form.on("Payment Entry", {
	...
	setup: function (frm) {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "cpmerp.controllers.queries.get_payment_terms_for_references",
					filters: {
	...
	refresh: function (frm) {
		cpmerp.hide_company(frm);
		frm.events.hide_unhide_fields(frm);
		frm.events.set_dynamic_labels(frm);
		frm.events.show_general_ledger(frm);
		cpmerp.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
	...
	contact_person: function (frm) {
		...
		cpmerp.utils.get_contact_details(frm);
	},
	...
	party_type: function (frm) {
		...
		frm.set_query("party", function () {
			if (frm.doc.party_type == "Employee") {
				return {
					query: "cpmerp.controllers.queries.employee_query",
				};
			}
		});
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			...
			return frappe.call({
				method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	set_account_currency_and_balance: function (
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", cpmerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", cpmerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		return frappe.call({
			method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	set_deductions_entry: function (frm, account) {
		if (frm.doc.difference_amount) {
			frappe.call({
				method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company,
				},
				callback: function (r, rt) {
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			frappe.call({
				method: "cpmerp.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
frappe.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
...
frappe.ui.form.on("Payment Entry", {
	cost_center: function (frm) {
		if (frm.doc.posting_date && (frm.doc.paid_from || frm.doc.paid_to)) {
			return frappe.call({
				method: "cpmerp.accounts.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
