...
cpmerp.accounts.taxes.setup_tax_validations("Sales Invoice");
cpmerp.accounts.payment_triggers.setup("Sales Invoice");
cpmerp.accounts.pos.setup("Sales Invoice");
cpmerp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
cpmerp.sales_common.setup_selling_controller();
cpmerp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	...
	company() {
		super.company();
		cpmerp.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	}
	onload() {
		var me = this;
		super.onload();
		...
		cpmerp.queries.setup_queries(this.frm, "Warehouse", function () {
			return cpmerp.queries.warehouse(me.frm.doc);
		});
		...
		cpmerp.queries.setup_warehouse_query(this.frm);
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				cpmerp.utils.map_current_doc({
					method: "cpmerp.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				cpmerp.utils.map_current_doc({
					method: "cpmerp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				cpmerp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "cpmerp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
				});
	...
	customer() {
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: cpmerp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new cpmerp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};
...
cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	cpmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	cpmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	cpmerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		(frm.custom_make_buttons = {
			...
			(frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function (doc, cdt, cdn) {
				return {
					query: "cpmerp.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "cpmerp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	company: function (frm) {
		if (frm.doc.company) {
			frappe.call({
				method: "cpmerp.setup.doctype.company.company.get_default_company_address",
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "cpmerp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "cpmerp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
					customer: frm.doc.customer,
					loyalty_program: frm.doc.loyalty_program,
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "cpmerp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
	...
	create_invoice_discounting: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm,
		});
	},
	...
	create_dunning: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm,
		});
	},
});
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "cpmerp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
