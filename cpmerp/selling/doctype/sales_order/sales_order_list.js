frappe.listview_settings["Sales Order"] = {
	...
	onload: function (listview) {
		var method = "cpmerp.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		listview.page.add_action_item(__("Sales Invoice"), () => {
			cpmerp.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
		});

		listview.page.add_action_item(__("Delivery Note"), () => {
			frappe.call({
				method: "cpmerp.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
				callback: (r) => {
					if (r.message) {
						...
						dialog.set_primary_action(__("Select"), function (values) {
							...
							cpmerp.bulk_transaction_processing.create(
								listview,
								"Sales Order",
								"Delivery Note",
								{
						...
						cpmerp.bulk_transaction_processing.create(listview, "Sales Order", "Delivery Note");
					}
				},
			});
		});
		...
		listview.page.add_action_item(__("Advance Payment"), () => {
			cpmerp.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
		});
	},
};
