...
frappe.provide("cpmerp.crm");
cpmerp.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			cpmerp.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			cpmerp.utils.map_current_doc({
				method: "cpmerp.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		cpmerp.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: cpmerp.utils.get_contact_details,
	...
	currency: function (frm) {
		let company_currency = cpmerp.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "cpmerp.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
cpmerp.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query("customer_address", cpmerp.queries.address_query);
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", cpmerp.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", cpmerp.queries["customer"]);
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "cpmerp.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new cpmerp.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new cpmerp.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new cpmerp.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "cpmerp.crm.doctype.opportunity.opportunity.get_item_details",
