...
frappe.ui.form.on("Company", {
	...
	setup: function (frm) {
		...
		cpmerp.company.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		cpmerp.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	delete_company_transactions: function (frm) {
		frappe.call({
			method: "cpmerp.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
					...
					frappe.verify_password(function () {
						var d = frappe.prompt(
							...
							function (data) {
								...
								frappe.call({
									method: "cpmerp.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
...
cpmerp.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return frappe.call({
			method: "cpmerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
cpmerp.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			cpmerp.company.set_custom_query(frm, v);
		}
	);
...
cpmerp.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
