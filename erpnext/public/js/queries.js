...
frappe.provide("cpmerp.queries");
$.extend(cpmerp.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "cpmerp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "cpmerp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "cpmerp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "cpmerp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "cpmerp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
cpmerp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
cpmerp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = cpmerp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "cpmerp.controllers.queries.warehouse_query" });
