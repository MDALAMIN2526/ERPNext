			...
			for item in self._items:
				...
				if hasattr(item, "valuation_rate") and hasattr(item, "gross_profit"):
					item.gross_profit = flt(
						(item.base_rate - item.get("valuation_rate", 0.0)) * item.stock_qty,
						item.precision("base_rate"),
					)
				item.stock_uom_rate = item.rate / (flt(item.conversion_factor) or 1.0)
				item.item_tax_amount = 0.0

	def calculate_item_pricing(self, item, effective_item_rate):
		# refactored to match js taxes_and_totals: apply_pricing_rule_on_item
		...
		item.rate = flt(rate_with_margin - item.discount_amount, item.precision("rate"))
